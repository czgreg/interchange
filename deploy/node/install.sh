...
install_leap() {
  ...
  CLIENT_SUBNET=$(grep -E '^\s+client_subnet:' /etc/leap/gateway.yaml | awk '{print $2}' | tr -d '"')
  TPROXY_PORT=$(grep -E '^\s+tproxy_port:' /etc/leap/gateway.yaml | awk '{print $2}' | tr -d '"')
  TUN0_IFACE="tun0"
  [ -n "$CLIENT_SUBNET" ] || fail "/etc/leap/gateway.yaml missing node.client_subnet"
  [ -n "$TPROXY_PORT" ]   || TPROXY_PORT=7893
  cat > /etc/leap/env <<EOF
# Generated by install.sh — do not edit manually; re-run install.sh to update.
export LEAP_CLIENT_SUBNET="$CLIENT_SUBNET"
export LEAP_TPROXY_PORT="$TPROXY_PORT"
export LEAP_TUN0_IFACE="$TUN0_IFACE"
EOF
  chmod 0644 /etc/leap/env
  log "env file written: subnet=$CLIENT_SUBNET tproxy_port=$TPROXY_PORT"
